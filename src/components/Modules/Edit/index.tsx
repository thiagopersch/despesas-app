...
type EditModulesProps = {
  ...
  modules?: ModulesForm | any;
  id?: string;
};

type Schema = z.infer<typeof schema>;

const EditModulesModal = ({
  handleClose,
  modules,
  handleSave,
  id,
  ...
  useEffect(() => {
    if (modules) {
      setValue('code', modules.code);
      setValue('name', modules.name);
      setValue('status', modules.status);
    }
  }, [modules, setValue]);
  ...
  const handleSaved: SubmitHandler<Schema> = async (values: ModulesForm) => {
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response) {
        if (error.response.status === 400) {
          setErrorMessage('Modules já existente');
        } else {
